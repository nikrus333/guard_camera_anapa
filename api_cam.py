...
class Application():
    ...
    def startDron(self):
        #if self.server.message == 'dron_start':
        data = "#start"
        ...
        #else:
            #mesb =messagebox.showinfo(title="Дрон", message="Дрон не готов к запуску, повторите операцию позже")
    ...
    def sendCord(self):
        if self.flag_dot == True:
            ...
            mesbox =messagebox.showinfo(title="Точка", message="Точка  не определена")
